...
function getActions() {
    ...
    $.ajax({
        url: '../ajax.php?action=get_actions',
        type: 'GET',
        data: {
            id: 1,
        },
        error: function () {
            alert('Something is wrong, couldn\'t get actions');
        },
        success: function (resp) {
            if (resp) {
                resp = JSON.parse(resp)

                displayActionsItems(resp);
                // displayMenuButtons();
            }
...
function displayActionsItems(actionsItems) {
    let displayActions = actionsItems.map(function (item) {
    ...
    displayActions = displayActions.join("");
    sectionCenter.innerHTML = displayActions;
}

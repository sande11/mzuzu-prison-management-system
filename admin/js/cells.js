...
function getCells() {
    ...
    $.ajax({
        url: '../ajax.php?action=get_cells',
        type: 'GET',
        data: {
            id: 1,
        },
        error: function () {
            alert('Something is wrong, couldn\'t get cells');
        },
        success: function (resp) {
            if (resp) {
                resp = JSON.parse(resp)

                displayCellsItems(resp);
                // displayMenuButtons();
            }
...
function displayCellsItems(cellsItems) {
    let displayCells = cellsItems.map(function (item) {
    ...
    displayCells = displayCells.join("");
    sectionCenter.innerHTML = displayCells;
}

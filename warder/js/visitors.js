...
function getVisitors() {
    ...
    $.ajax({
        url: '../ajax.php?action=get_visitors',
        type: 'GET',
        data: {
            id: 1,
        },
        error: function () {
            alert('Something is wrong, couldn\'t get visitors');
        },
        success: function (resp) {
            if (resp) {
                resp = JSON.parse(resp)

                displayVisitorsItems(resp);
                // displayMenuButtons();
            }
...
function displayVisitorsItems(visitorsItems) {
    let displayVisitors = visitorsItems.map(function (item) {
        return `<tr class="table-row">
                                                ...
                                                <td>
                                                 <button class="btn" onclick="displayInmateInfo(${item.id})" ><i class="fas fa-edit"></i></button>
                                                <button class="btn" onclick="archiveAppointment(${item.booking_id})" ><i class="fas fa-archive"></i></button></td>
                                            
                    </tr>
                     `;
    });
    displayVisitors = displayVisitors.join("");
    sectionCenter.innerHTML = displayVisitors;
}
...
function getInmates() {
    $.ajax({
        ...
        success: function (resp) {
            ...
            var select = document.getElementById("myInmates"); // Get the select element
            ...
            options.forEach(option => {
                ...
                optionElement.text = option.inmate; // Set the text content of the option element
